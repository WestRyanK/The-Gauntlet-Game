...
{
    this->vertex_data = mlVertex(vertex.vertex_data);
...
{
    unsigned int index = this->triangles.size();
...
void LoopSubdivider::adjust_even_vertices(Mesh* new_mesh, Mesh* old_mesh)
{
    for (int i = 0; i < old_mesh->vertices.size(); i++ )
    {
        ...
        if (adjacent_vertices.size() > 3)
            beta = 3.0/(8.0 * adjacent_vertices.size());
        else
            beta = 3.0/16.0;
...
Mesh* LoopSubdivider::create_mesh_from_mlmesh(mlMesh* input_mesh)
{
    ...
    {
        Vertex vertex;
        vertex.vertex_data = mlVertex(input_vertex);
        mesh->insert_vertex(vertex);
    }

    for (int i = 0; i < input_mesh->indices.size(); i += 3)
    {
        Triangle triangle;
        for(int j = 0; j < 3; j++)
        {
