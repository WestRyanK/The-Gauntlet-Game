...
void GridCollisionDetector::create_grid()
{
    ...
    this->x_count = ceil(x_size / this->grid_size);
    this->y_count = ceil(y_size / this->grid_size);
    this->z_count = ceil(z_size / this->grid_size);
...
{
    if (location.x >= this->x_count || location.x < 0
     || location.y >= this->y_count || location.y < 0
     || location.z >= this->z_count || location.z < 0)
    {
        return NULL;
    }
    else
